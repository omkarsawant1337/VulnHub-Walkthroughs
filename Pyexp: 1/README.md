...
After brute-forcing the MariaDB root account, access to the database was obtained. Database enumeration revealed Fernet-encrypted credentials and the corresponding encryption key. These credentials were successfully decrypted and used to authenticate via SSH as user **lucy**. 
...
Because user input was passed directly to `exec()`, arbitrary Python code could be executed with root privileges. A payload spawning `/bin/sh` resulted in a root shell. 
